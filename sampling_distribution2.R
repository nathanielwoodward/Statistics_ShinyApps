    ...
    sidebarPanel(
      ...
      div(
        ...
        sliderInput("iter1","Iterations:",
                    value=5000,min=1,max=5000,step=10,animate=T),
...
observeEvent(input$select, {
  ...
  if(input$select=="right") {
    ...
    updateSliderInput(session, "sd1", value = 1.5, min = 1, max=5, step=.5)
...
dataInput <- reactive({
  ...
  output$sampdistn <- renderPlot(height=300,{
   ...
   p<-ggplot()+geom_histogram(aes(y=..density..,x= dataInput()$means[1:input$iter1]), color="gray50",bins=100)+
      theme_light()+ggtitle(paste0("\nSampling Distribution of the Mean (N = ",input$n1,")"))+
      annotate("text",x=-Inf,y=Inf, hjust="left", vjust="top",
               label=paste0("\n   mean = ",suppressWarnings(round(mean(dataInput()$means[1:input$iter1]),2)),"\n",
                            "   sd = ",round(sd(dataInput()$means[1:input$iter1]),3)), size=5)+ylab("")+xlab(bquote(bar(x)))
    

    if(input$empsamp){
     p<-p+ stat_function(geom="line", fun=dnorm, args=list(mean=input$mean1,sd=input$sd1/sqrt(input$n1)),size=1,color="blue")+
       ylab("")+xlab(bquote(bar(x)))
       
    }
    
      if(input$scale){    
      if(input$select=="norm") p #+xlim(input$mean1-3*input$sd1, input$mean1+3*input$sd1)
      else if(input$select=="unif") p #+xlim(input$mean1-input$sd1*sqrt(3), input$mean1+input$sd1*sqrt(3))
      else if(input$select=="right") p #+xlim(0,qlnorm(.999,m,s))
      else if(input$select=="left") p #+xlim(0,1)
      else if (input$select=="bimod") p
      }
      else p<-p+xlim(rv$xlims)
